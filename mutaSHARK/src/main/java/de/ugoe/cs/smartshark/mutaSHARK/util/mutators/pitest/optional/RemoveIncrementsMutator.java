...
public class RemoveIncrementsMutator extends PitestMutator
{
    @Override
    public List<MutatedNode> getPossibleMutations(TreeNode treeNode, TreeNode target, List<Action> actions)
    {
        ...
        results.addAll(getPossibleDecrementRemoveMutations(treeNode, target, actions));
        results.addAll(getPossibleIncrementRemoveMutations(treeNode, target, actions));
        return results;
    }

    private List<MutatedNode> getPossibleDecrementRemoveMutations(TreeNode treeNode, TreeNode target, List<Action> actions)
    {
        ArrayList<MutatedNode> results = new ArrayList<>();
        for (int i = 0; i < actions.size(); i++)
        {
            if (actions.get(i) instanceof Delete)
            {
                ...
                if (!deletePrefix.getNode().getType().name.equals("PrefixExpression") && !deletePrefix.getNode().getType().name.equals("PostfixExpression"))
                {
                    continue;
                }
                ...
                if (!(deletePrefix.getNode().getChildren().get(0).getType().name.equals("PREFIX_EXPRESSION_OPERATOR") && (deletePrefix.getNode().getChildren().get(0).getLabel().equals("--"))) && !(deletePrefix.getNode().getChildren().get(1).getType().name.equals("POSTFIX_EXPRESSION_OPERATOR") && (deletePrefix.getNode().getChildren().get(1).getLabel().equals("--"))))
                {
                    continue;
                }
                    ...
                    if (actions.get(j) instanceof TreeDelete)
                    {
                        ...
                        for (int k = 0; k < actions.size(); k++)
                        {
                            if (actions.get(k) instanceof TreeInsert)
                            {
                                ...
                                results.add(new MutatedNode(clonedTree, this, 25, "Removed -- @~" + parent.getPos()));
                            }
                        }
                    }
                }
    ...
    private List<MutatedNode> getPossibleIncrementRemoveMutations(TreeNode treeNode, TreeNode target, List<Action> actions)
    {
        ArrayList<MutatedNode> results = new ArrayList<>();
        for (int i = 0; i < actions.size(); i++)
        {
